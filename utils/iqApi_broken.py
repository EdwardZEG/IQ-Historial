...
class IQOptionAPI:
    ...
    def connect(self, email, password):
        try:
            ...
            if check:
                ...
                return {"success": False, "error": f"Fallo en conexion: {reason}"}
    ...
    def get_history(self, account_type="PRACTICE", start_date=None, end_date=None, instrument="all"):
        try:
            ...
            if not start_date:
                start_date = end_date - (30 * 24 * 60 * 60)  # 30 dias atras
            else:
                try:
                    start_date = int(
            ...
            # print(f"[INFO] Buscando operaciones desde {datetime.datetime.fromtimestamp(start_date)} hasta {datetime.datetime.fromtimestamp(end_date)}")

            # Intentar diferentes metodos para obtener historial REAL
            history = []
            methods_tried = []
            
            try:
                # print("[INFO] Intentando metodo: get_position_history_v2")
                ...
                if success and data:
                    ...
                    # print(f"[SUCCESS] get_position_history_v2 devolvio {len(history)} operaciones")
                # else:
                    # print(f"[WARNING] get_position_history_v2 no devolvio datos: success={success}")
            except Exception as e:
                # print(f"[ERROR] get_position_history_v2 fallo: {e}")
                pass
                
            if not history:
                try:
                    # print("[INFO] Intentando metodo: get_position_history")
                    ...
                    if success and data:
                        ...
                        # print(f"[SUCCESS] get_position_history filtrado devolvio {len(history)} de {len(all_positions)} operaciones")
                    # else:
                        # print(f"[WARNING] get_position_history no devolvio datos: success={success}")
                except Exception as e:
                    # print(f"[ERROR] get_position_history fallo: {e}")
                    pass
                    
            if not history:
                try:
                    # print("[INFO] Intentando metodo: get_positions")
                    ...
                    if success and data:
                        ...
                        # Filtrar por fecha
                        history = [pos for pos in all_positions if pos.get('created', 0) >= start_date and pos.get('created', 0) <= end_date]
                        # print(f"[SUCCESS] get_positions filtrado devolvio {len(history)} de {len(all_positions)} operaciones")
                    # else:
                        # print(f"[WARNING] get_positions no devolvio datos: success={success}")
                except Exception as e:
                    # print(f"[ERROR] get_positions fallo: {e}")
                    pass

            if history is None:
                history = []

            # print(f"[INFO] Total de metodos intentados: {methods_tried}")
            # print(f"[INFO] Operaciones encontradas: {len(history)}")

            # Formatear los datos REALES
            formatted_history = []
            current_balance = self.api.get_balance()
            
            # print(f"[INFO] Balance actual: ${current_balance}")

            for i, trade in enumerate(history):
                try:
                    # Depurar la primera operacion para ver la estructura
                    # if i == 0:
                        # print(f"[DEBUG] Estructura de la primera operacion:")
                    ...
                    # print(f"[ERROR] Error al formatear operacion {i}: {e}")
                    # print(f"Operacion problematica: {trade}")
                    pass
            
            # print(f"[SUCCESS] Operaciones REALES formateadas: {len(formatted_history)}")
            ...
            if instrument != "all" and instrument:
                ...
                # print(f"[FILTER] Filtro de instrumento '{instrument}': {original_count} -> {len(formatted_history)} operaciones")
            ...
            if formatted_history:
                ...
                # print(f"[STATS] Estadisticas calculadas:")
                # print(f"   - Total operaciones: {estadisticas['total_operaciones']}")
                # print(f"   - Inversion total: ${estadisticas['inversion_total']:.2f}")
                ...
                # print(f"   - Porcentaje de exito: {estadisticas['porcentaje_exito']:.1f}%")
            else:
                estadisticas = {
                    "total_operaciones": 0,
                    "inversion_total": 0,
                ...
                # print("[WARNING] No se encontraron operaciones reales en el periodo especificado")
                # print("[INFO] Sugerencia: Verifica las fechas o realiza algunas operaciones en IQ Option")
    ...
    def disconnect(self):
        if self.api and self.connected:
            try:
                self.connected = False
                # print("[INFO] Desconectado de IQ Option")  # Comentado para evitar interferir con JSON
            except:
                pass
